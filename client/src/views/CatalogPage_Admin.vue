                        ...
                        <router-link v-for="book in  filteredBooks.slice(selectedPage*(nbBooksPerPage - booksNotVisible),(selectedPage+1)*nbBooksPerPage)"  :key = "books.indexOf(book)" :class="[isAdmin ? 'book-page-link1' : (recomandations ? 'book-page-link2' : 'book-page-link')]"  :to="{ path: 'book-page', query: { ref: book.reference }}">
                            ...
                            <div  :class="[recomandations ? 'book1' : 'book']" >
                                <div>
                                    ...
                                    <img :src="addressServer+'/' + book.image_src" alt="book_pic" class="book-cover">
                                </div>
    ...
    methods: {
        ...
        addBooks(newBooks){
            ...
            newBooks.forEach(book => {
                axios.post(this.addressServer+"/api/livre/store", book).then((response) => {
        ...
        async getAllGenres() {
            try {
                const response = await axios.get(this.addressServer+'/api/genre/all');
        ...
        async getFilteredBooks() {
            ...
            await axios.post(this.addressServer+"/api/livre/filter", filters).then((response) => {
                    if (response.status === 200) {
        ...
        async deleteBookAPI(id) {
            await axios.delete(this.addressServer+"/api/livre/delete/" + id ,{  
                params: {
                    token:this.token
                }
            }).then((response) => {
